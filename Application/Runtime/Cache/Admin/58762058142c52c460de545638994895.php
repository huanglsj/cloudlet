...
<div class="container-fluid">
	...
	<div class="row-fluid form-wrapper">
		<form id="_form" class="form" method="post"
			action="<?php echo U(Super/calendar);?>">
			<input type="hidden" name="opflg" id="opflg" value="<?php echo ($opflg); ?>" />
			<table table data-toggle="table" data-url="data.json">
				<thead>
					<tr>
						<td style="width: 12%; text-align: center">商品</td>
						<td><select class="span6" name="goodspid" style="width: 100px" onchange="reload(this.value)" value="<?php echo ($pid); ?>">
							<?php if(is_array($goodlist)): $i = 0; $__LIST__ = $goodlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['pid']); ?>" <?php if($pid == $vo['pid'] ): ?>selected<?php endif; ?>><?php echo ($vo['ptitle']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select></td>
						<td style="width: 12%; text-align: center">年份</td>
						<td><input type="text" class="span6 search" name="year" id="year" value="<?php echo ($year); ?>" /></td>
						<td>
						    <?php if($_SESSION['pkey']['CALENDAR_ADD'] == '1'): ?><input type="submit" class="btn-glow primary" value="生成" /><?php endif; ?>
						</td>
					</tr>
				</thead>
			</table>
		...
		<br />
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$monthinfo): $mod = ($i % 2 );++$i;?><div class="calendarDiv">
			<table class="calendarTb">
				<input type="hidden" name="month" value="<?php echo ($monthinfo['month']); ?>" />
				<tr>
					<td colspan="7" style="background-color: #DCDCDC"><?php echo ($monthinfo['month']); ?>月</td>
				</tr>
				<tr >
                    ...
                    <?php if(is_array($monthinfo['monthList'])): $k = 0; $__LIST__ = $monthinfo['monthList'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dayinfo): $mod = ($k % 7 );++$k; if(($mod) == "0"): ?><tr><?php endif; ?>
                        <td>
                        <?php if($dayinfo['day'] != ''): ?><div name ="day" data-trigger="focus"
                                <?php if($dayinfo['isworkday'] == '0'): ?>class="holiday dayDiv"<?php endif; if($dayinfo['isworkday'] == '1'): ?>class="dayDiv"<?php endif; ?>
                                 data-html="true"  data-content="<a href= &quot;<?php echo U('Super/editworkday');?>?fulldate=<?php echo ($dayinfo['fulldate']); ?>&pid=<?php echo ($pid); ?>&quot;><i class='table-edit'></i></a>"
                                  data-container="body" data-toggle="popover" data-placement="right" ><?php echo ($dayinfo['day']); ?>
                                <input type="hidden" name="day" value="<?php echo ($dayinfo['day']); ?>"/>
                            </div><?php endif; ?>
                        </td>
                        <?php if(($mod) == "6"): ?></tr><?php endif; ?>
                        <?php if($k == count($monthinfo['monthList'])): $__FOR_START_1170367773__=count($monthinfo['monthList']);$__FOR_END_1170367773__=42;for($ik=$__FOR_START_1170367773__;$ik < $__FOR_END_1170367773__;$ik+=1){ if($ik%7 == 0): ?><tr><?php endif; ?>
                                <td>&nbsp</td>
                                <?php if($ik%7 == 6): ?></tr><?php endif; } endif; endforeach; endif; else: echo "" ;endif; ?>
			</table>
...
<script type="text/javascript">
	...
	function reload(value) {
		window.location.href = "<?php echo U('Super/calendar');?>?pid=" + value + "&year="
				+ $("#year").val();
	}

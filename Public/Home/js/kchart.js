...
function appendUrl(origin,appender){

	//如果结尾是.html，则取出后再添加

	if(origin.endWith(".html")){
...
var ChartControl = {
    ...
    **/
	...
	console.log('1');
        ...
        $(".ul_four li a").bind("click",function(){
...
console.log($(this).data("interval"))
            ...
            if(ChartControl.chartType == "area")
                ...
                ChartControl.speed = 1000;

            else

                ChartControl.speed = ChartControl.chartInterval*60*1000;

            if(ChartControl.timer){

            	window.clearInterval(ChartControl.timer);
        ...
        $(".ul_four li a").eq(1).trigger("click");
    ...
    */
    ...
    getKChartData:function(_type){
        ...
        var _url = appendUrl(this.initUrl,"/interval/"+ChartControl.chartInterval);
    ...
    */
    ...
    onRealTime:function(){
        ...
        $.ajax({
            ...
            url:appendUrl(ChartControl.realTimeUrl,"/interval/"+ChartControl.chartInterval+"/curtime/"+ChartControl.maxTime),
            ...
            success:function(result){
                ...
                if(result){
...

			        {
			            ...
			            // if(data[0].time != ChartControl.chartData[ChartControl.chartData.length-1].time)
...

			            {
...

			                {
			                    ...
			                    ChartControl.chartData.push([data[0].time*1,data[0].price*1]);
...

			                {
...

			                    {
			                        ...
			                        for(var i = option.series[1].data.length-1;i>option.series[1].data.length-6;i--)

			                        {

			                            count += option.series[1].data[i];

			                        }

			                        ma5 = (count/5).toFixed(2);
			                    ...
			                    option.series[1].data.push(ma5*1);

			

			                    //MA10均线

			                    if(option.series[2].data.length >= 5)

			                    {

			                        var count = 0;

			                        for(var i = option.series[2].data.length-1;i>option.series[2].data.length-6;i--)

			                        {

			                            count += option.series[2].data[i];

			                        }

			                        ma10 = (count/5).toFixed(2);
			                    ...
			                    option.series[2].data.push(ma10*1);

			

			                    //MA30均线

			                    if(option.series[3].data.length >= 5)

			                    {

			                        var count = 0;

			                        for(var i = option.series[3].data.length-1;i>option.series[3].data.length-6;i--)

			                        {

			                            count += option.series[3].data[i];

			                        }

			                        ma30 = (count/5).toFixed(2);
			                    ...
			                    option.series[3].data.push(ma30*1);
    ...
    */
	...
	console.log(this.fullDate[this.fullDate.length-1]);
...
/*    createKChrt:function(_dataConfig)

    {
        ...
        option.yAxis[0].boundaryGap = true;
...
*/
...

	{
        ...
        option.series.push({
            ...
            data:ChartControl.GetSeriesDataByDateRange(5,ChartControl.chartData)
        ...
        option.series.push({
            ...
            data:ChartControl.GetSeriesDataByDateRange(10,ChartControl.chartData)
        ...
        option.series.push({
            ...
            data:ChartControl.GetSeriesDataByDateRange(30,ChartControl.chartData)
        ...
        */
        ...
        ChartControl.timer = setInterval(function(){
    ...
    */
	...
	console.log(_dataConfig.categoryData.length)
	...
	console.log(parseInt(option.xAxis[0].data.length/7));

        option.xAxis[0].axisLabel.interval = parseInt(option.xAxis[0].data.length/8);

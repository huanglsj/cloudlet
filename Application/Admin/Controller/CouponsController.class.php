...
class CouponsController extends Controller {
    ...
    public function cpsend()
    {
        ...
        $user = D('userinfo');
        $cpall = $exper->select();
        $ulist = $user->field('uid,username')->select();
        $ucount = $user->count();
        ...
        if(IS_POST){
            ...
            if($experienceinfo->create()){
                ...
                else if($data['uid']=="member"){
                    //选择了会员单位
                    //首先判断是否选择了代理
                    if(I('post.member') <> 'daili'){
                        //全部代理
                        $memberuserlst = M('userinfo')->where('companyid='.I('post.member'))->field('uid')->select();
                        if(empty($memberuserlst)){
                            $this->error('没有发放对象，请重新选择后再发放！');
                        }
                        foreach($memberuserlst as $key=>$value)
                        {
                            $data['uid'] = $value['uid'];
                            $result = $experienceinfo->add($data);
                        }
            ...
            $memberlist = D('companyinfo')->where("isdelete='N'")->field('cid,comname')->select();

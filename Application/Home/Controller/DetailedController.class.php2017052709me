...
class DetailedController extends CommonController {
    ...
    public function newOrder(){
        $uid=$_SESSION['uid'];
        $order=M('order')->field('oid')->where("uid={$uid} and ostaus=0")->select();
        $this->ajaxReturn($order);
    }

    // 持仓明细
    public function checkOrderIs(){
        $oid = I('get.oid');
        $order=M('order')->field('ostaus')->where('oid='.$oid)->select();
        $this->ajaxReturn($order);
    }
    ...
    public function drevenue(){
        $uid=$_SESSION['uid'];
        //$count =M('order')->where('uid='.$uid)->count();
...
//        $page->parameter = array('type','as'); //此处的row是数组，为了传递查询条件
        ...
        $list = M('order')->where(array('uid'=>$uid))->order('buytime desc' )->limit($page->firstRow.','.$page->listRows)->select();
        $this->assign('list',$list);
        $this->assign('page',$show);
...
//            die("<script>alert('休市期间不能下单！');history.go(-1);</script>");
            $this->ajaxReturn(array("info"=>"休市期间不能下单！","icon"=>3));
			return;
        }
        if ($data['eid']==1) {
            $data['selltime']='';
            ...
            if($isopen == false){
                ...
                $this->ajaxReturn(array("info"=>"很快就要休市了，请您选择一个较短时间周期下单，速战速决吧","icon"=>3));
...
//            die("<script>alert('余额不足，请充值！');location.href='".U('/Home/User/deposit')."';</script>");
        ...
        if($userinfo['ustatus'] == 3){
            $this->ajaxReturn(array("info"=>"您的账户异常，请稍后重试","icon"=>0));
...
//            die("<script>alert('交易机构目前繁忙，请稍后再试！');history.go(-1);</script>");
            $this->ajaxReturn(array("info"=>"所属交易机构保证金缴纳不足，请联系您的邀请人补充资金后再交易","icon"=>0));
...
//            die("<script>alert('超过最大单笔建仓金额！');history.go(-1);</script>");
        ...
        if($keepamount + $pay > $maxuseramount){
            $this->ajaxReturn(array("info"=>"超过最大持仓金额","icon"=>0));
        ...
        if ($result) {
            ...
            if($ortn){
                ...
                if($result['result']){
                    ...
                    //根据下单金额做风控
                    if($data['fee'] > 0){
                        $modelRisk = D('Common/Risk');
                        $infoRisk = $modelRisk->getRiskResult($data['fee']);
                        if($infoRisk['is_lost'] == 1){
                            $pcdata['ostyle'] = $data['ostyle'];
                            $pcdata['is_lost'] = 1;
                            $pcdata['lost_time'] = $pcdata['close_time'] ? $pcdata['close_time'] : (time() + rand(60,120)); //如果点数模式就在60-120秒之间平仓
                            $pcdata['lost_ext'] = $data;
                            $pcdata['lost_ext']['oid'] = $ortn;
                            if($pcdata['ostyle'] == 1){ //买跌
                                if($data['order_type'] == 1){ //按点数
                                    $pcdata['lost_ext']['lost_point'] = $pcdata['close_price_high'];
                                }else{
                                    $pcdata['lost_ext']['lost_point'] = ($data['buyprice'] + rand(1,2));
                                }
                            }else{
                                if($data['order_type'] == 1){ //按点数
                                    $pcdata['lost_ext']['lost_point'] = $pcdata['close_price_low'];
                                }else{
                                    $pcdata['lost_ext']['lost_point'] = ($data['buyprice'] - rand(1,2));
                                }
                            }
                            $pcdata['lost_ext'] = json_encode($pcdata['lost_ext']);
                        }
                    }

                    if(!M('waitcloseorder')->add($pcdata)){
                        $this->ajaxReturn(array("info"=>"买入失败","icon"=>2));
//                        die("<script>alert('买入失败！');history.go(-1);</script>");
                    }else{
                        D('Common/RealtimepriceUser')->addData($pcdata);  //添加用户模拟行情
                    }
...
//                            die("<script>alert('买入失败！');history.go(-1);</script>");
                    ...
                    $this->ajaxReturn(array("info"=>"买入成功","icon"=>1,"reload"=>1));
                    //die("<script class="PopupBgTint">alert('买入成功！');history.go(-1);</script>");
                }else{
//                    die("<script>alert('".$result['msg']."');history.go(-1);</script>");
                    $this->ajaxReturn(array("info"=>$result['msg'],"icon"=>0));
                }
            }else{
                $this->ajaxReturn(array("info"=>"买入失败","icon"=>2));
//                die("<script>alert('买入失败！');history.go(-1);</script>");
...
//      $order->ploss=$youjia-$myorder['buyprice'];
    ...
    public function getKchartData(){
        ...
        if($interval == 0){
            ...
            //K线图，获取30个点
            $subQuery = M('k'.$interval.'price')->where('pid='.$pid.' and isopen=1')->order('updatetime desc')->field("open,close,low as lowest,high as highest,starttime,updatetime*1000 as time,FROM_UNIXTIME(updatetime, '%Y-%m-%d %H:%i:%s') as fulltime")->limit('0,45')->cache(true,0.5)->select(false);
        }
        $model = new \Think\Model();
        $data = $model->table($subQuery.' a')->order('time asc')->select();

        if($_SESSION['uid'] && $pid){
            $data = D('Common/RealtimepriceUser')->getKline($data, $interval, $_SESSION['uid'], $pid, NOW_TIME, 1);
        }


        //$_SESSION['uid'];
/*        if($_SESSION['uid'] && $pid){
            $listRU = D('Common/RealtimepriceUser')->getlistByUid($_SESSION['uid'], $pid, NOW_TIME, 1);
            //var_dump($listRU);exit;
            if($listRU){
                foreach($data as $key=>&$val){
                    foreach($listRU as $key2=>$val2){
                        if($interval == 0){
                            ...
                            if(($val['time'] / 1000) >= $val2['dealtime']){ //接近真实行情时间的替换
...
//            $data = M('realtimeprice')->where('pid='.$pid.' and dealtime > '.$curtime.' and isopen=1')->order('dealtime desc')->field("newprice as price,openPrice as open,lastClosePrice as close,lowprice as lowest,highprice as highest,dealtime*1000 as time,FROM_UNIXTIME(dealtime, '%Y-%m-%d %H:%i:%s') as fulltime")->limit('0,1')->cache(true,0.5)->select();
            $data = M('realtimeprice')->where('pid='.$pid.' and isopen=1')->order('dealtime desc')->field("newprice as price,openPrice as open,lastClosePrice as close,lowprice as lowest,highprice as highest,dealtime*1000 as time,FROM_UNIXTIME(dealtime, '%Y-%m-%d %H:%i:%s') as fulltime")->limit('0,1')->select();
        }else{
//            $data = M('k'.$interval.'price')->where('pid='.$pid.' and updatetime > '.$curtime.' and isopen=1')->order('updatetime desc')->field("open,close,low as lowest,high as highest,updatetime*1000 as time")->limit('0,1')->cache(true,0.5)->select();
            $data = M('k'.$interval.'price')->where('pid='.$pid.' and isopen=1')->order('updatetime desc')->field("open,close,low as lowest,high as highest,updatetime*1000 as time,FROM_UNIXTIME(updatetime, '%Y-%m-%d %H:%i:%s') as fulltime")->limit('0,1')->select();
        }


        if($_SESSION['uid'] && $pid){
            $data = D('Common/RealtimepriceUser')->getKline($data, $interval, $_SESSION['uid'], $pid, NOW_TIME, 2);
        }


 /*       if($_SESSION['uid'] && $pid){
            $listRU = D('Common/RealtimepriceUser')->getlistByUid($_SESSION['uid'], $pid, NOW_TIME, 2);

            foreach($data as $key=>&$val){
                foreach($listRU as $key2=>$val2){
                    if($interval == 0){

                        if($val2['dealtime'] >= ($val['time'] / 1000)){
                            if($val['highest'] < $val2['newprice']){
                                $val['highest'] = $val2['newprice'];
                            }elseif($val['lowest'] > $val2['newprice']){
                                $val['lowest'] = $val2['newprice'];

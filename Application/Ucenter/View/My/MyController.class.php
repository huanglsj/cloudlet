...
class MyController extends CommonController {
    ...
    public function personinfo(){
        ...
        if (IS_POST) {
            ...
            $result = $companyinfo->where('cid='.$cid)->setField(array('managername'=>$managername,'managerphone'=>$managerphone));
            if($result){
                $this->success('修改成功!');
            }else{
                $this->error("修改失败！");
            ...
            if($userinfo){
                ...
                $company = $companyinfo->where('deptid='.$deptInfo['id'])->find();
                    $this->error($deptInfo['code']);
retrun;
	       $aliuser = $alipayinfo->where('code='.$deptInfo['code'])->find();
                if($deptInfo){
                    //获取上级单位信息
                ...
                $url=  $hostlink.U('User/reg')."?invitecode=".$deptInfo['invite_code'];

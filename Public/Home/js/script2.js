$(document).ready(function(){
	$(".sw_active a").click();
	var index = 0;
	  var rs = $("#xianjia").eq(index).get(0).getAttribute("value");
	  if (rs==1) {
	  	//油
	  	code='conc';
	  	//code='hf_cl';
      ...
      $.ajax({  
        ...
        success: function(data) {
          ...
          //现在
   		  ...
   		  //今日差价
   		  var chajia=data[0]-data[8];
   		  var zhangfu=chajia/data[0]*100;
   		  $('#ycj').html((chajia).toFixed(2));
   		  $('#zf').html((zhangfu).toFixed(2)+'%');
   		  //document.write(zhangfu.toFixed(2)+'%');
          //var sum= data[12];
          if(data[0]<yprice){
            $('#xianjia').attr("class","drop");
          }else if(data[0]==yprice){}else{
            $('#xianjia').attr("class","rise");
          }              
        },  
        }); 
     };

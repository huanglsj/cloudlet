            ...
            <div class="row-fluid header">
                <h3>统计报表</h3>
	                <div class="span10 pull-right">
	                    <div class="tpsearch">
	                    	<label class="span4">报表类型：</label><select id="reporttype" class="span6" name="reporttype">
	                    				<option value="">请选择</option>
		                                <option value="kehuzijinliushui">资金流水</option>
		                                <option value="kehuzijinzhuangkuang">资金状况</option>
		                                <option value="jiaoyimingxi">交易明细</option>
		                                <option value="jiaoyihuizong">交易汇总</option>
		                                <option value="youhuiquan">优惠券</option>
	                    			</select>
	                    </div>
	                </div>
	                <div class="baobiao kehuzijinliushui" style="display:none">
	               	 <form action="<?php echo U('Tongji/exportExcel');?>" method="post">
	               	 	<input type='hidden' value=kehuzijinliushui name="reporttype">
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                    <div class="tpsearch">
		                    	<label class="span5">客户账号：</label><input  class="span7" type="text" name="username" id="username"/>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">操作类型：</label><select id="otype" class="span7" name="otype">
		                    				<option value="">请选择</option>
			                                <option value="1">充值</option>
			                                <option value="2">提现</option>
			                                <option value="3">建仓</option>
			                                <option value="4">平仓</option>
			                                <option value="100">手动修改</option>
		                    			</select>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">下属会员单位：</label><select id="member" class="select2 span7" name="member">
		                    				<option value="0">请选择</option>
		                    				<?php if(is_array($subdepts)): $i = 0; $__LIST__ = $subdepts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dept): $mod = ($i % 2 );++$i;?><option value="<?php echo ($dept['code']); ?>"><?php echo ($dept['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		                    			</select>
		                    </div>
		                </div>
		                 <div class="span10 pull-right" style="margin-top: 20px;">
		                    <div class="tpsearch">
		                    	<label class="span5">查询开始日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作开始日期..." value='<?php echo ($startTime); ?>' name="startTime" id="odate_start"/>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">查询结束日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作结束日期..." value='<?php echo ($endTime); ?>' name="endTime" id="odate_end"/>
		                    </div>
		                </div>
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                	<input type="submit" class="btn-glow primary" value="导出报表">
		                </div>
		                </form>
	                </div>
	                <div class="baobiao kehuzijinzhuangkuang" style="display:none">
	               	 <form action="<?php echo U('Tongji/exportExcel');?>" method="post">
	               	 	<input type='hidden' value=kehuzijinzhuangkuang name="reporttype">
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                    <div class="tpsearch">
		                    	<label class="span5">客户账号：</label><input  class="span7" type="text" name="username" id="username"/>
		                    </div>
	                ...
	                <div class="baobiao jiaoyimingxi" style="display:none">
	                	<form action="<?php echo U('Tongji/exportExcel');?>" method="post">
	                	<input type='hidden' value="jiaoyimingxi" name="reporttype">
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                     <div class="tpsearch">
		                    	<label class="span5">产品信息：</label><select id="product" class="span7" name="product">
		                    				<option value="">请选择</option>
		                    				<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$product): $mod = ($i % 2 );++$i;?><option value="<?php echo ($product["pid"]); ?>"><?php echo ($product["displayname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		                    			</select>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">交易模式：</label><select id="eid" class="span7" name="eid">
		                    				<option value="">请选择</option>
			                                <option value="1">点数</option>
			                                <option value="2">时间</option>
		                    			</select>
		                    </div>
		                     <div class="tpsearch">
		                    	<label class="span5">盈亏状态：</label><select id="ploss" class="span7" name="ploss">
		                    				<option value="">请选择</option>
		                 ...
		                 <div class="span10 pull-right" style="margin-top: 20px;">
		                    ...
		                    <div class="tpsearch">
		                    	<label class="span5">结算开始日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作开始日期..." value='<?php echo ($startTime); ?>' name="startTime" id="startTime"/>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">结算结束日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作结束日期..." value='<?php echo ($endTime); ?>' name="endTime" id="endTime"/>
		                    </div>
		                </div>
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                	<input type="submit" class="btn-glow primary" value="导出报表">
                	...
                	<div class="baobiao jiaoyihuizong" style="display:none">
	                	<form action="<?php echo U('Tongji/exportExcel');?>" method="post">
	                	<input type='hidden' value="jiaoyihuizong" name="reporttype">
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                     <div class="tpsearch">
		                    	<label class="span5">产品信息：</label><select id="product" class="span7" name="product">
		                    				<option value="">请选择</option>
	                ...
	                <div class="baobiao youhuiquan" style="display:none">
	                	<form action="<?php echo U('Tongji/exportExcel');?>" method="post">
	                	<input type='hidden' value="youhuiquan" name="reporttype">
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                     <div class="tpsearch">
		                    	<label class="span5">发放渠道：</label><select id=""sendtype"" class="span7" name="sendtype">
		                    				<option value="">请选择</option>
			                                <option value="1">注册赠送</option>
			                                <option value="2">后台发放</option>
		                    			</select>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">优惠券面额：</label><select id="eid" class="span7" name="eid">
		                    				<option value="">请选择</option>
		                    				<?php if(is_array($exprinfo)): $i = 0; $__LIST__ = $exprinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$expr): $mod = ($i % 2 );++$i;?><option value="<?php echo ($expr["eid"]); ?>">有效期<?php echo ($expr["limittime"]); ?>天<?php echo ($expr["eprice"]); ?>元券</option><?php endforeach; endif; else: echo "" ;endif; ?>
		                    			</select>
		                    </div>
		                     <div class="tpsearch">
		                    	<label class="span5">盈亏状态：</label><select id="ploss" class="span7" name="ploss">
		                 ...
		                 <div class="span10 pull-right" style="margin-top: 20px;">
		                    ...
		                    <div class="tpsearch">
		                    	<label class="span5">使用开始日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作开始日期..." value='<?php echo ($startTime); ?>' name="startTime" id="startTime"/>
		                    </div>
		                    <div class="tpsearch">
		                    	<label class="span5">使用结束日期：</label><input type="text" data-date-format="yyyy-mm-dd" class="input-large datepicker span7" placeholder="请输入操作结束日期..." value='<?php echo ($endTime); ?>' name="endTime" id="endTime"/>
		                    </div>
		                </div>
		                <div class="span10 pull-right" style="margin-top: 20px;">
		                	<input type="submit" class="btn-glow primary" value="导出报表">
...
<script type="text/javascript">
    $(function () {
        ...
        $('#reporttype').change(function (){
        	...
        	if(this.value=='kehuzijinliushui'){
        		$('.kehuzijinliushui').css("display","block");
        	}
        	else if(this.value=='kehuzijinzhuangkuang'){
        		$('.kehuzijinzhuangkuang').css("display","block");
        	}else if(this.value=='jiaoyimingxi'){
        		$('.jiaoyimingxi').css("display","block");
        	}else if(this.value=='jiaoyihuizong'){
        		$('.jiaoyihuizong').css("display","block");
        	}else if(this.value=='youhuiquan'){

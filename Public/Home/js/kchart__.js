...
console.log($(this).data("interval"))
    ...
    */
    getKChartData:function(_type){
        ...
        var _url = appendUrl('http://hq.gz.1251923837.clb.myqcloud.com/HQ/Goods2/AG?t='+Date.parse(new Date()));
        $.ajax({
            url:_url,
            type:"get",
            dataType:"jsonp",
    ...
    */
    onRealTime:function(){
        var data = null;
        $.ajax({
            url:appendUrl('http://hq.gz.1251923837.clb.myqcloud.com/HQ/Goods2/AG?t='+Date.parse(new Date())),
            type:"get",
            dataType:"jsonp",
            async:true,//异步
                ...
                if(result){
			        ...
			        {        
			            ...
			            if(data)
			            {
			                ...
			                ChartControl.maxTime = time;
			                if(ChartControl.chartType == "area")
			                {
                                
			                    ChartControl.chartData.push([time*1,data.c*1]);
			                    //追加数据点
			                    ChartControl.fullDate.push(time);
			                    var option = ChartControl.areaChart.getOption();
			                    var xLabel = new Date(time*1).Format("hh:mm");
			                    //保证相同刻度不要显示
    ...
    formatterData:function(_data)
    {
	   ...
	   console.log('2');
        ...
        for (var i = 0; i < _data.length; i++) {
            this.fullDate.push(fulltime);
            time = Date.parse(new Date());
            categoryData.push(time*1);
            //开收低高
            kData.push([_data[i].c*1,_data[i].close*1,_data[i].c*1,_data[i].cb*1]);
            areaData.push(_data[i].c*1);
        }

...
<div class="container-fluid text-center" >
            <div id="pad-wrapper" class="users-list">
                <div class="row-fluid header">
                    <h3>实名认证申请列表</h3>
                    <!-- 
                    <form id="searchfm" method="post" action="<?php echo U('User/ulist');?>">
                ...
                <div class="row-fluid table">
                    <table class="table table-hover">
                        ...
                        <tr class="first">
                            ...
                            <td>
                                <?php echo ($ult['IDnumber']); ?>
                            </td>
                            <td>
                                <?php echo ($ult['mobile']); ?>
                            </td>
                            <td>
                            	<?php if($ult['IDcardfront'] == ''): else: ?>
                                  <img src="/Uploads/<?php echo ($ult['IDcardfront']); ?>" class="img-thumbnail avatar hidden-phone" 
                                        onmouseover="this.style.cursor='pointer';this.style.cursor='hand'" onmouseout="this.style.cursor='default'"
                                        onclick="javascript:showimage('/Uploads/<?php echo ($ult['IDcardfront']); ?>');"
                                  /><?php endif; ?>
                            </td>
                            <td>
                                <?php if($ult['IDcardback'] == ''): else: ?>
                                  <img src="/Uploads/<?php echo ($ult['IDcardback']); ?>" class="img-thumbnail avatar hidden-phone" 
                                        onmouseover="this.style.cursor='pointer';this.style.cursor='hand'" onmouseout="this.style.cursor='default'"
                                        onclick="javascript:showimage('/Uploads/<?php echo ($ult['IDcardback']); ?>');"
                                  /><?php endif; ?>
                            </td>
                            <td>
                                <?php if($ult['IDcardhandheld'] == ''): else: ?>
                                  <img src="/Uploads/<?php echo ($ult['IDcardhandheld']); ?>" class="img-thumbnail avatar hidden-phone" 
                                        onmouseover="this.style.cursor='pointer';this.style.cursor='hand'" onmouseout="this.style.cursor='default'"
                                        onclick="javascript:showimage('/Uploads/<?php echo ($ult['IDcardhandheld']); ?>');"
                                  /><?php endif; ?>
                            </td>
                            <td>
                                <?php echo (date('Y-m-d',$ult['applytime'])); ?>
                            </td>
                            <td>
                                <?php if($ult['status'] == '1'): ?>申请中<?php endif; ?>
                                <?php if($ult['status'] == '2'): ?>已通过<?php endif; ?>
                                <?php if($ult['status'] == '3'): ?><span id="reasonshow" rel="popover" data-content="拒绝原因：<?php echo ($ult['ngreason']); ?>" data-trigger="hover" data-placement="left">已拒绝</span><?php endif; ?>
                            </td>
                            <td>
                                <?php if($ult['status'] == '1'): ?><ul class="actions">
                            	    <?php if($_SESSION['pkey']['USER_AUTHEN_YES'] == '1'): ?><li><a href="<?php echo U('User/editauthentications');?>?uid=<?php echo ($ult['uid']); ?>&msgFlg=checkok&status=2&useruid=<?php echo ($ult['useruid']); ?>" onclick="if(confirm('确定要通过吗?')){return true;}else{return false;}">通过</a></li><?php endif; ?>
                                    <?php if($_SESSION['pkey']['USER_AUTHEN_NG'] == '1'): ?><li class="last">
                                        <!-- <a href="<?php echo U('User/editauthentications');?>?uid=<?php echo ($ult['uid']); ?>&status=3&useruid=<?php echo ($useruid); ?>" onclick="if(confirm('确定要拒绝吗?')){return true;}else{return false;}">拒绝</a> -->
                                         <a  data-toggle="modal" data-target="#reasonModal" data-backdrop="static" onclick="showreasonmodal(<?php echo ($ult['uid']); ?>)">拒绝</a>
                                         </li><?php endif; ?>
                                </ul><?php endif; ?>
                            </td>
...
<script type="text/javascript">
   ...
   function showimage(source)
	...
	function doNG(){
		...
		window.location.href = "<?php echo U('User/editauthentications');?>?uid=" + nguid + "&status=3&useruid=<?php echo ($useruid); ?>&ngreason="+reason;
	}

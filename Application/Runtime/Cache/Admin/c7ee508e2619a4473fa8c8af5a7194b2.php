...
<div class="container-fluid">
  <form method="get" action="<?php echo U('balancelist');?>" name="searchForm">
      ...
      <div class="row-fluid header">
          ...
          <div class="span10 pull-right">
              <div class="tpsearch">
              	...
              	<select id="ostyle" class="span6" name="optype">
                    ...
                    <option value="1" <?php if($optype==1)echo 'selected' ?>>充值</option>
                    <option value="2" <?php if($optype==2)echo 'selected' ?>>提现</option>
                    <option value="3" <?php if($optype==3)echo 'selected' ?>>建仓</option>
                    <option value="4" <?php if($optype==4)echo 'selected' ?>>平仓</option>
                    <option value="100" <?php if($optype==100)echo 'selected' ?>>手动修改</option>
              	</select>
              </div>
              <div class="tpsearch">
              	<div class="cond-label">所属机构编码</div>
              	<input type="text" class="span6 search" name="compcode" value="<?php echo ($compcode); ?>"/>
              </div>
              <div class="tpsearch">
          ...
          <div class="span10 pull-right" style="margin-top:10px;margin-bottom:10px">
              <div class="tpsearch">
              	...
              	<input type="text" class="input-large" style="margin-bottom: 0;" name="opdate" value="<?php echo ($opdate); ?>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})">
              </div>
              <div class="tpsearch">
              	<div class="cond-label">金额</div>
              	<input type="text" class="span6 search money" name="amount1" style="width:70px" value="<?php echo ($amount1); ?>"/>~
              	<input type="text" class="span6 search money" name="amount2" style="width:70px" value="<?php echo ($amount2); ?>"/>
              </div>
	          <div class="tpsearch">
        ...
        <div class="row-fluid table">
            <table class="table table-hover">
                ...
                <tbody id="ajaxback">
				<?php if(is_array($balancelist)): $i = 0; $__LIST__ = $balancelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$balance): $mod = ($i % 2 );++$i;?><!-- row -->
                <tr class="first">
                    <td><?php echo ($i); ?></td>
					<td>
					  <?php if(($balance["optype"] == '1') or ($balance["optype"] == '2') ): ?><a href="<?php echo U('User/rechargedetail', array('id'=>$balance['opid']));?>"><?php echo ($balance["id"]); ?></a>
                      <?php elseif(($balance["optype"] == '3') or ($balance["optype"] == '4') ): ?>
                        <a href="<?php echo U('Order/ocontent', array('oid'=>$balance['opid']));?>"><?php echo ($balance["id"]); ?></a>
                      <?php else: ?>
                        <?php echo ($balance["id"]); endif; ?>
                    </td>
                    <td>
                        <a href="<?php echo U('User/updateuser',array('uid'=>$balance['uid']));?>"><?php echo ($balance["customer"]); ?></a>
                    </td>
                    <td><?php echo ($balance["huiyuan_code"]); ?></td>
                    <td><?php echo ($balance["daili_name"]); ?></td>
                    <td><?php echo ($balance["jigou_name"]); ?></td>
                    <td>
                    	<?php if($balance["optype"] == '1' ): ?>充值
                        <?php elseif($balance["optype"] == '2' ): ?>
                        提现
                        <?php elseif($balance["optype"] == '5' ): ?>`
                        提现驳回
                        <?php elseif($balance["optype"] == '6' ): ?>
                        提现失败
                        <?php elseif($balance["optype"] == '3' ): ?>
                        建仓
                        <?php elseif($balance["optype"] == '4' ): ?>
                        平仓
                        <?php elseif($balance["optype"] == '100' ): ?>
                        手动修改<?php endif; ?>
                    </td>
                    <td><?php echo ($balance["amount"]); ?></td>
                    <td><?php echo ($balance["balance"]); ?></td>
                    <td><?php echo (date('Y-m-d H:i',$balance["optime"])); ?></td>
                    <td>
                    	<?php if($balance["optype"] == '100' ): echo ($balance["opusername"]); ?>
                        <?php else: ?>
                        -<?php endif; ?>
                    </td>
                </tr>

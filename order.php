...
do{
/*
	$content1 = file_get_contents("http://120.27.24.252/xh/json.php?code=cl1000xh");//得到文件执行的结果
...
*/
    ...
    //查询所有的没有平仓的订单
    if (date("H")==4) {
       $array = $db->ExecuteDQL("select * from wp_order where  ostaus=0 ");
       $db->ExecuteDML("UPDATE wp_webconfig SET isopen=0 WHERE id=1");
    }

    if (date("His")>=80000&&date("His")<=80010) {
       $db->ExecuteDML("UPDATE wp_webconfig SET isopen=1 WHERE id=1");
    }

	   //获取当前时间的油价
	$ydiancha=selectcid(1);
	$source1=file_get_contents("D:/wwwroot/heimao/xh/you.txt");
 ...
 //    echo $youjia;
	...
	$source2=file_get_contents("D:/wwwroot/heimao/xh/baiyin.txt");
 ...
 //   echo $yinjia;
	...
	$source3=file_get_contents("D:/wwwroot/heimao/xh/tong.txt");
   ...
   // echo $tongjia;
    ...
    foreach ($arraynow as $now) {
     //   var_dump($now['pid']);
        ...
        $order =  $db->ExecuteDQL("select * from wp_productinfo where pid=".$now['pid']);
        ...
        else if($cid==3)
        {
            ...
            if ($now['ostyle']==0 && $sellprice!=0 && $sellprice!=null) {

                if($sellprice!=0||$sellprice!=null)
                {
                  $ykzj = ($sellprice-$now['buyprice'])*$now['onumber']*$wav;
            ...
            if($ykzj>0)
               ...
               $array=$db->ExecuteDQL("select * from wp_order where  ostaus=0 and ostyle=0 and endprofit!=0  and endprofit<=".$ypr);
                
               }
             if($ykzj<0)
               {

                $array=$db->ExecuteDQL("select * from wp_order where  ostaus=0 and ostyle=0 and endloss!=0 and endloss<=".$ypr);
            ...
            if ($now['ostyle']==1 && $sellprice!=0 && $sellprice!=null) {
                //盈亏资金
                $ykzj =($sellprice-$now['buyprice'])*$now['onumber']*$wav;
                //本单盈余
                $bdyy = round($jc+$ykzj,1);
            ...
            if($ykzj<0)
               {
                ...
                $array=$db->ExecuteDQL("select * from wp_order where  ostaus=0 and ostyle=1 and endprofit!=0 and  endprofit<=".$ypr);
    
               }

             if($ykzj>0)
               {
               $array=$db->ExecuteDQL("select * from wp_order where  ostaus=0 and ostyle=1  and endloss!=0 and  endloss<=".$ypr);
               }
